...
const towerDefs = [
  { id: "violet", name: "Violet", cost: 2, damage: 2, range: 0.374, fireRate: 0.935, color: "#7c3aed", coneDegrees: 220, maxTargets: 1 },
  ...
  { id: "red", name: "Red", cost: 7, damage: 3.3, range: 0.434, fireRate: 0.945, color: "#dc2626", coneDegrees: BASE_TOWER_CONE_DEGREES, maxTargets: 1 },
  { id: "green", name: "Green", cost: 9, damage: 4, range: 0.635, fireRate: 1.265, color: "#22c55e", coneDegrees: BASE_TOWER_CONE_DEGREES * 0.9, maxTargets: 1 },
  { id: "orange", name: "Orange", cost: 13, damage: 4, range: 0.443, fireRate: 1.375, color: "#f97316", coneDegrees: 200, maxTargets: 2 }
];
...
function chooseAIBatchPattern(playerDefenseScore, waveNumber) {
  ...
  if (playerDefenseScore > 8) {
    return ["tank", "brute", "wisp", "imp", "runner"];
  }
  return ["imp", "runner", "brute", "tank", "wisp", "runner"];
}
...
function prepareAIMoves() {
  ...
  // Allow at least one expensive tower per round when there's mana to spare,
  // so towers like Orange (cost 13) can actually be considered. Without this
  // floor, defenseRatio caps the budget below the most expensive tower's cost
  // unless mana exceeds ~60.
  const expensiveTowerCost = 13;
  if (state.aiMana >= 20 && defenseBudget < expensiveTowerCost && attackBiasFloor >= expensiveTowerCost) {
    defenseBudget = expensiveTowerCost;
  }
  ...
  for (const unit of state.attackersPlayer) {
    ...
    if (unit.poisonTicksRemaining > 0) {
      ...
      if (unit.poisonTickTimer <= 0) {
        ...
        if (typeof window.__harnessOnPoisonTick === "function") {
          window.__harnessOnPoisonTick(unit, dotDamage);
        }
      }
    }
    if (unit.hp <= 0) {
